...
- **一键进入 RSXM（GUI 默认）**：打开 APK → 自动解压 Alpine Linux → 启动 reasonix 交互会话，**默认进入原生 GUI 会话视图**（字段映射气泡 + 原生输入框）；快捷栏「视图」或返回键可切回 CLI 终端；退出 reasonix 后落到 Alpine shell。
...
- **纯黑主题（无标题栏）**：全局纯黑界面——已去除顶部标题栏，主屏直接是快捷入口行 + 终端/GUI 内容区（侧滑菜单入口移至快捷栏「菜单」），视觉保持纯黑。
- **主屏快捷入口行（精简）**：常用功能（菜单 / GitHub / 视图切换 / ADB / API Key / DS2API / 更新 / 项目 / 会话 / 设置）以快捷栏形式直接显示在主屏顶部，一键直达；低频功能收进「设置」二级菜单。
